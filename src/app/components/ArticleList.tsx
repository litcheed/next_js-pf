            ...
            {articles.map((article) => (
                <article className='shadow my-4 flex flex-col' key={article.id}>
                    ...
                    <div className='bg-white flex flex-col justify-start p-6'>
                        ...
                        <Link href={`articles/${article.id}`} className='text-slate-900 pb-6'>{article.content}</Link>
                        <Link href={`articles/${article.id}`} className='text-green-400 hover:text-black'>続きを読む</Link>
                    </div>
                </article>
            ))}
        </div>
